module PageTests exposing (suite)
...
suite =
    PagesProgram.describe "End-to-end page tests"
        [ loginFlow
        , todoApp
        , searchFilter
        , counter
...
loginFlow : PagesProgram.Test
loginFlow =
    PagesProgram.test "login flow with validation and HTTP auth"
...
todoApp : PagesProgram.Test
todoApp =
    PagesProgram.test "todo app: add, complete, delete"
        (PagesProgram.fromConfig
            { data = BackendTask.succeed ()
            , init =
                \() ->
...
searchFilter : PagesProgram.Test
searchFilter =
    PagesProgram.test "search filter"
        (PagesProgram.fromConfig
            { data =
                BackendTask.succeed
                    { items =
...
counter : PagesProgram.Test
counter =
    PagesProgram.test "counter"
        (PagesProgram.fromConfig
            { data = BackendTask.succeed ()
            , init = \() -> ( { count = 0 }, [] )
            , update =
