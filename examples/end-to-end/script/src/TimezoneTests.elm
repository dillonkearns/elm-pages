...
import BackendTaskTest exposing (testTask)
import Date
import Expect
import FatalError
...
{-| The system-timezone-dependent tests require TZ=America/New\_York.
...
run : Script
run =
    BackendTask.Env.get "TZ"
        |> BackendTask.andThen
            (\maybeTz ->
                if maybeTz == Just "America/New_York" then
                    BackendTask.succeed ()
                    ...
                    FatalError.build
                        { title = "Wrong TZ"
                        , body =
                            "These timezone tests require TZ=America/New_York but got TZ="
                                ++ Maybe.withDefault "(not set)" maybeTz
                                ++ ".\n\nRun with:\n    TZ=America/New_York npx elm-pages run script/src/TimezoneTests.elm"
                        }
                        |> BackendTask.fail
            )
        |> BackendTask.andThen
            (\() ->
                BackendTaskTest.describe "Timezone"
