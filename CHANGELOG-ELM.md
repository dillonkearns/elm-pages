...
- Fix compiler error with generated scaffold code in `Scaffold.Form`.
...
- `BackendTask.File.exists` to check whether a file exists.
- `BackendTask.File.optional` to attempt a file read and return `Nothing` instead of failing.
...
- Add `Script.readKey` and `Script.readKeyWithDefault` for reading single keypresses in scripts without requiring Enter.
- Add `File.binaryFile`, for reading binary files.
- Allow using `elm-codegen` version `6`.
