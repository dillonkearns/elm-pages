 ...
 * Users import TestApp and pass route configs to Test.PagesProgram.start.
 * @param {string[][]} templates
 * @returns {string}
 */
function testAppModule(templates) {
  // Only include static routes (no dynamic segments like Slug_)
  const staticOnly = templates.filter((name) => {
    return !name.some((part) => part.endsWith("_"));
  });

  const routeImports = staticOnly
    .map((name) => `import Route.${name.join(".")}`)
    .join("\n");

  const exposing = staticOnly
    .map((name) => routeConfigName(name))
    .join(", ");

  if (staticOnly.length === 0) {
    return `module TestApp exposing (..)

{-| Generated test configurations. No static routes found. -}
`;
  }

  // Only generate configs for static routes (no dynamic segments)
  const staticTemplates = templates.filter((name) => {
    return !name.some((part) => part.endsWith("_"));
  });

  const routeConfigs = staticTemplates
    .map((name) => {
      const moduleName = `Route.${name.join(".")}`;
      const configName = routeConfigName(name);
      const routeParamsExpr = routeParamsDefault(name);

      return `
{-| Test config for \`${moduleName}\`. Pass to \`Test.PagesProgram.start\`.

    import TestApp
    import Test.PagesProgram as PagesProgram

    PagesProgram.start TestApp.${configName}
...
${configName} =
    let
        routeParams = ${routeParamsExpr}

        defaultShared = Tuple.first (Shared.template.init (Pages.Flags.PreRenderFlags) Nothing)

        extractEffects effect =
            case effect of
                Effect.None -> []
                Effect.Batch effects -> List.concatMap extractEffects effects
                _ -> []

        app pageData =
            Test.PagesProgram.Route.makeApp
                { data = pageData
                , sharedData = ()
    ...
    in
    { data =
        ${moduleName}.route.data Test.PagesProgram.Route.testRequest routeParams
            |> BackendTask.andThen Test.PagesProgram.Route.unwrapResponse
    , init =
        \\pageData ->
            let
                ( model, effect ) = ${moduleName}.route.init defaultShared (app pageData)
            in
            ( model, extractEffects effect )
    , update =
        \\msg model ->
            let
                -- Note: update receives a dummy app; pageData is not available here.
                -- For most routes this is fine since update rarely uses app.data.
                ( newModel, effect, _ ) = ${moduleName}.route.update (app (crashData ())) msg model defaultShared
            in
            ( newModel, extractEffects effect )
    , view =
        \\pageData model ->
            let
                v = ${moduleName}.route.view defaultShared model (app pageData)
            in
            { title = v.title
            , body = List.map (Html.Styled.toUnstyled >> Html.map (always (crashData ()))) v.body
            }
    }`;
    })
    .join("\n\n");

  return `module TestApp exposing (${exposing})

{-| Generated test configurations for each route module.
Import this module and pass route configs to \`Test.PagesProgram.start\`.
 ...
 * @returns {string}
 */
function routeParamsDefault(name) {
  // For now, return empty record. Dynamic routes will need params.
  return "{}";
}
