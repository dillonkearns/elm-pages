...
function testAppModule(templates) {
  const routeImports = templates
    .map((name) => `import Route.${name.join(".")}`)
    .join("\n");

  const exposing = templates
...
`;
  ...
  const routeConfigs = templates
    .map((name) => {
      const moduleName = `Route.${name.join(".")}`;
      const configName = routeConfigName(name);

      return `
{-| Test config for \`${moduleName}\`. Pass route params and use with
\`Test.PagesProgram.start\`.

    import TestApp
    import Test.PagesProgram as PagesProgram

    PagesProgram.start (TestApp.${configName} ${hasDynamicSegments(name) ? "{ slug = \"my-post\" }" : "{}"})
        |> PagesProgram.ensureViewHas [ ... ]
        |> PagesProgram.done

-}
${configName} routeParams =
...
Each route config is a function that takes route params:

    -- Static route (no params)
    PagesProgram.start (TestApp.index {})

    -- Dynamic route
    PagesProgram.start (TestApp.blogSlug_ { slug = "hello" })
...
{-| Project-specific configuration shared across all routes.
...
projectConfig =
    ...
    , viewToHtml =
        \\v ->
            { title = v.title
            , body = List.map (Html.Styled.toUnstyled >> Html.map (\\_ -> crashPlaceholder ())) v.body
            }
    }


extractEffects : Effect msg -> List (BackendTask FatalError msg)
