...
function spawnLocalElmPages() {
    return new Promise((resolve) => {
        const { status, stderr, stdout } = spawnSync("node_modules/.bin/elm-pages", ["run", "src/TestScript.elm"]);
        resolve({
            status,
...
describe.sequential("runElmCodegenInstall", () => {
    ...
    describe("via elm-pages run", () => {
        ...
        it("succeeds", async () => {
            ...
            await expect(spawnLocalElmPages()).resolves.toEqual(expect.objectContaining({
                status: 0,
                stderr: "",
    ...
    describe("with elm-codegen missing", () => {
        ...
        it("returns a descriptive error", async () => {
            ...
            await expect(runElmCodegenInstall()).resolves.toEqual(expect.objectContaining({
                success: false,
