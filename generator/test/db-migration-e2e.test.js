...
describe("E2E: single migration (V1 → V2)", () => {
  it("full flow: seed → migrate → implement → detect → validate → read → write", async () => {
    ...
    expect(stubContent).toContain("module Db.Migrate.V2 exposing (db)");
    expect(stubContent).toContain("todo_implement_migration_V1_to_V2");

    // Verify MigrateChain.elm created
    ...
    expect(chainContent).toContain("import Db.V1");
    expect(chainContent).toContain("import Db.Migrate.V2 as MigrateV2");
...
`
    ...
    // === Step 5: MigrateChain for V3 should handle both V1 and V2 ===
    const chainContent = fs.readFileSync(
      path.join(tmpDir, ".elm-pages-db", "MigrateChain.elm"),
      "utf8"
    );
    expect(chainContent).toContain("import Db.V1");
    expect(chainContent).toContain("import Db.V2");
    expect(chainContent).toContain("import Db.Migrate.V2 as MigrateV2");
    expect(chainContent).toContain("import Db.Migrate.V3 as MigrateV3");
    expect(chainContent).toContain("migrateFromV1");
    expect(chainContent).toContain("migrateFromV2");
...
`
    ...
    // Should decode V1 and V2 data
    expect(chainContent).toContain("Db.V1.w3_decode_Db");
    expect(chainContent).toContain("Db.V2.w3_decode_Db");
    // Should chain through migrations
    expect(chainContent).toContain("MigrateV2.db");
    expect(chainContent).toContain("MigrateV3.db");
...
describe("E2E: generateMigrateChain content verification", () => {
  it("single migration chain is valid Elm structure", () => {
    const chain = generateMigrateChain(2);

    // Module declaration
    expect(chain).toMatch(/^module MigrateChain exposing \(run\)/m);
    ...
    // Run function
    expect(chain).toContain("run : BackendTask FatalError ()");
    expect(chain).toContain("run =");
    expect(chain).toContain("readDbBin");

    // Migration function
    expect(chain).toContain("migrateFromV1 : Db.V1.Db -> BackendTask FatalError ()");
    expect(chain).toContain("saveAndLog (MigrateV2.db model)");

    // Save function
    expect(chain).toContain("saveAndLog : Db.Db -> Int -> Int -> BackendTask FatalError ()");
    expect(chain).toContain("Wire.bytesEncode (Db.w3_encode_Db newDb)");
    expect(chain).toContain("Base64.fromBytes wire3Bytes");

    // Read function
    expect(chain).toContain("readDbBin : BackendTask FatalError { version : Int, bytes : Bytes }");

    // Error handling
    expect(chain).toContain("other ->");
    expect(chain).toContain("Unknown db.bin version");
  });

  it("multi-step chain chains migrations correctly", () => {
    const chain = generateMigrateChain(4);

    // Should have V1, V2, V3 snapshots
    expect(chain).toContain("import Db.V1");
    expect(chain).toContain("import Db.V2");
    expect(chain).toContain("import Db.V3");

    // Should have V2, V3, V4 migrations
    expect(chain).toContain("import Db.Migrate.V2 as MigrateV2");
    expect(chain).toContain("import Db.Migrate.V3 as MigrateV3");
    expect(chain).toContain("import Db.Migrate.V4 as MigrateV4");

    // migrateFromV1 should chain to migrateFromV2
    expect(chain).toContain("migrateFromV1 model =");
    expect(chain).toContain("migrateFromV2 (MigrateV2.db model)");

    // migrateFromV2 should chain to migrateFromV3
    expect(chain).toContain("migrateFromV2 model =");
    expect(chain).toContain("migrateFromV3 (MigrateV3.db model)");

    // migrateFromV3 should save (terminal migration)
    expect(chain).toContain("migrateFromV3 model =");
    expect(chain).toContain("saveAndLog (MigrateV4.db model)");
  });
});
