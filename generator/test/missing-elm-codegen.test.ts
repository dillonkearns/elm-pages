...
function buildPathWithoutElmCodegen(pathValue) {
  const delimiter = process.platform === "win32" ? ";" : ":";
  const pathEntries = (pathValue ?? "").split(delimiter).filter(Boolean);
  const hasElmCodegen = (entry) =>
    existsSync(`${entry}/elm-codegen`) || existsSync(`${entry}/elm-codegen.cmd`);

  return pathEntries.filter((entry) => !hasElmCodegen(entry)).join(delimiter);
}
...
describe.sequential("runElmCodegenInstall", () => {
  ...
  describe("with elm-codegen missing", () => {
    beforeEach(() => {
      // Simulate an environment where `elm-codegen` is unavailable by removing
      // every PATH entry that contains the executable.
      process.env.PATH = buildPathWithoutElmCodegen(originalPATH);
    });
    afterEach(() => {
