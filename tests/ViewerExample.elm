...
For the full page-preview experience, run `elm-pages dev` and open
`/_tests`, which serves the HTML shell that syncs snapshots into the
preview iframe.
...
main =
    Viewer.app
        [ ( "Blog: full user journey", blogFullJourney )
        , ( "Login flow", loginFlowTest )
        , ( "Todo app", todoAppTest )
        , ( "Blog: loads posts from API", blogLoadsPostsTest )
        , ( "Counter with model inspector", counterTest )
        , ( "Search with filtering", searchTest )
        ]
...
blogFullJourney : List PagesProgram.Snapshot
blogFullJourney =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
            { data = Blog.data
            , init = Blog.init
            , update = Blog.update
            , view = Blog.view
...
loginFlowTest : List PagesProgram.Snapshot
loginFlowTest =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
...
todoAppTest : List PagesProgram.Snapshot
todoAppTest =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
            { data = BackendTask.succeed ()
        ...
        , PagesProgram.withinFind
            [ PSelector.tag "li", PSelector.containing [ PSelector.text "Learn Elm" ] ]
...
blogLoadsPostsTest : List PagesProgram.Snapshot
blogLoadsPostsTest =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
            { data = Blog.data
...
counterTest : List PagesProgram.Snapshot
counterTest =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
            { data = BackendTask.succeed ()
...
searchTest : List PagesProgram.Snapshot
searchTest =
    PagesProgram.snapshots
        (PagesProgram.fromConfig
            { data =
