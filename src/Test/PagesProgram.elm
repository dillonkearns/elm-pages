...
expects a `TestApp.ProgramTest` pipeline -- the test framework calls
[`done`](#done) for you. The exposed value is a `Test.PagesProgram.Test`,
and `elm-pages` discovers it by name (typically `suite`).
...
run via the `elm-pages test-run` CLI or viewed in the
...
Build a named test tree just like `elm-test`. [`test`](#test) wraps a
`ProgramTest` pipeline with a string name; [`describe`](#describe) groups
related tests under a shared heading. Use [`toTest`](#toTest) when you
want to run the same tree through plain `elm-test` without the
`elm-pages` wrapper CLI; [`toNamedSnapshots`](#toNamedSnapshots) flattens
the tree for the visual runner.
...
pressKey key selectors programTest =
    dispatchKeyDown ("pressKey \"" ++ key ++ "\"") key selectors programTest
...
dispatchFormSubmitForInput inputSelectors programTest =
    ...
    tryDispatchEvent { targetRequired = False }
        [ Selector.tag "form"
...
{-| Finalize a `ProgramTest` pipeline into an `Expect.Expectation`. Fails
if any earlier assertion failed, or if there are unresolved pending
BackendTask effects.

The usual path is to wrap pipelines with [`test`](#test) and let the
test framework call `done` for you. Reach for `done` directly when you
need a raw `Expectation` -- for example when you are already inside a
plain `elm-test` `test "..." <| \() -> ...` block.
...
{-| Wrap a single `ProgramTest` pipeline with a descriptive string name.
...
Unlike module-level binding names, the string can be a full sentence and
is what shows up in `elm-test` output and in the visual runner sidebar.

-}
test : String -> ProgramTest model msg -> Test
test label programTest =
    SingleTest label (toOutcome programTest)
...
`elm-pages test-run` works with `Test.PagesProgram.Test` directly. Use
...
toOutcome programTest =
    Outcome
        { expectation = done programTest
        , snapshots = toSnapshots programTest
        }
