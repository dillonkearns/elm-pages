...
- **[`Tui`](https://package.elm-lang.org/packages/dillonkearns/elm-pages/12.2.0/Tui)** — build interactive terminal apps as elm-pages scripts. TEA-style `init`/`update`/`view` that renders styled text to the terminal and responds to keyboard, mouse, paste, resize, and time events. `Tui.programOrScript` lets the same script behave interactively in a TTY and as a plain `BackendTask` under CI or when its output is piped.
- **[`Test.PagesProgram`](https://package.elm-lang.org/packages/dillonkearns/elm-pages/12.2.0/Test-PagesProgram)** — end-to-end tests for elm-pages routes. Drive the real framework runtime through form submissions, optimistic UI, concurrent fetchers, redirects, session cookies, and error pages with simulated HTTP and custom ports. Deterministic, in-process, no browser. Inspired by [elm-program-test](https://package.elm-lang.org/packages/avh4/elm-program-test/latest/).
- **[`Test.Tui`](https://package.elm-lang.org/packages/dillonkearns/elm-pages/12.2.0/Test-Tui)** — pure Elm tests for TUI programs. Simulate keypresses and mouse events, resolve pending `BackendTask`s with the [`Test.BackendTask`](https://package.elm-lang.org/packages/dillonkearns/elm-pages/12.2.0/Test-BackendTask) simulators, and assert on screen output.
- **Visual test runner** — `elm-pages test-view` steps through page tests in the browser and `elm-pages test` steps through TUI tests in the terminal, both showing recorded snapshots with highlighted selectors. Run either headlessly with `elm-pages test-run` via `elm-test`.
...
- [elm-pages Elm API Docs](https://package.elm-lang.org/packages/dillonkearns/elm-pages/12.2.0/)
